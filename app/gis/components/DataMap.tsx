...
export default function VoiceMap({ points, initialView }: MapProps) {
  ...
  const [hoveredPoint, setHoveredPoint] = useState<DataTraffic | null>(null);
  ...
  return (
    <div className="w-full h-[600px] rounded-2xl overflow-hidden border">
      ...
      >
        ...
        {points.map((p) => {
          const colorClass = getMarkerColor(p.datarevenue);
          return (
            <Marker
              key={p.name}
              latitude={p.latitude}
        ...
        {hoveredPoint && (
          ...
          >
            <div className="text-sm space-y-1">
              ...
              {hoveredPoint.data3gtraffic !== null && (
                <div>Data 3G Traffic: {hoveredPoint.data3gtraffic}</div>
              )}
              {hoveredPoint.data4gtraffic !== null && (
                <div>Data 4G Traffic: {hoveredPoint.data4gtraffic}</div>
              )}
